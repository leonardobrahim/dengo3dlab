...
const regex = /<div className="space-y-6 pt-2">[\s\S]*?\{\/\* Dynamic Calculated Specs Summary Pill \*\/\}/g;
...
// We should also remove the old Dynamic Calculated Specs Summary Pill content completely, because selectedSize doesn't exist anymore
const specRegex = /\{\/\* Dynamic Calculated Specs Summary Pill \*\/\}[\s\S]*?\{\/\* Stock Availability Alert \*\/\}/g;

const emptySpec = `{/* Stock Availability Alert */}`;

content = content.replace(specRegex, emptySpec);

// Replace selectedColor in Stock Availability Alert
const alertRegex = /Esta combinação de <strong>\{selectedColor\.name\}<\/strong> está esgotada no momento\./g;
const newAlert = `Esta variação está esgotada no momento.`;
