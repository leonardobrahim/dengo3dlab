...
import { Check, CheckCircle2, Circle, ArrowRight, Package, Truck, Box, Receipt, Cpu } from 'lucide-react';
...
export const OrderTimeline: React.FC<OrderTimelineProps> = ({
  ...
  return (
    <div className={cn('space-y-6 text-left relative', className)}>
      {/* Background Line */}
      <div className="absolute left-[15px] top-6 bottom-6 w-0.5 bg-slate-100 dark:bg-slate-800 z-0" />
      ...
      {isCancelled ? (
        <div className="relative z-10 flex items-start gap-4 p-4 bg-red-50 dark:bg-red-950/20 rounded-2xl border border-red-100 dark:border-red-900/30 text-red-600 dark:text-red-400">
          ...
          <div>
            ...
            <p className="text-xs mt-1">Este pedido foi cancelado e não será processado.</p>
          </div>
        </div>
      ) : (
        <div className="space-y-6 relative z-10">
          {baseSteps.map((step, idx) => {
            ...
            const isCompleted = currentLevel > stepLevel || (currentLevel === stepLevel && currentLevel === 5); // delivered is always complete when reached
            ...
            if (step.key === 'pending') matchingEvent = order.shipment?.events.find(e => e.status.includes('Criado'));
            if (step.key === 'confirmed') matchingEvent = order.shipment?.events.find(e => e.status.includes('Pagamento') || e.status.includes('Confirmado'));
            if (step.key === 'in_production') matchingEvent = order.shipment?.events.find(e => e.status.includes('Produção'));
            if (step.key === 'ready_to_ship') matchingEvent = order.shipment?.events.find(e => e.status.includes('Separação') || e.status.includes('Preparação'));
            if (step.key === 'shipped') matchingEvent = order.shipment?.events.find(e => e.status.includes('Enviado') || e.status.includes('Trânsito'));
            if (step.key === 'delivered') matchingEvent = order.shipment?.events.find(e => e.status.includes('Entregue'));
            ...
            } else if (step.key === 'delivered' && order.shipment?.deliveredAt) {
               dateStr = formatDate(order.shipment.deliveredAt);
               timeStr = formatTime(order.shipment.deliveredAt);
            }

            return (
              <div key={step.key} className={cn("flex items-start gap-4", isFuture ? "opacity-50" : "")}>
                ...
                <div className="flex-1 min-w-0 pt-1.5 pb-2">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-4">
                    <h4 className={cn("text-sm font-bold", isCurrent ? "text-pink-600 dark:text-pink-400" : isFuture ? "text-slate-400" : "text-slate-900 dark:text-slate-100")}>
                      {step.label}
                    </h4>
                    {(dateStr || timeStr) && (
                  ...
                  {isCurrent && step.key === 'in_production' && (
                    <div className="mt-3 p-3 bg-pink-50/50 dark:bg-pink-950/20 rounded-xl border border-pink-100 dark:border-pink-900/30">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          <p className="text-xs font-bold text-slate-900 dark:text-slate-100">Seu produto está sendo produzido.</p>
                          <p className="text-[11px] text-slate-600 mt-0.5">Previsão: 2–4 dias úteis.</p>
                        </div>
                      </div>
                    </div>
                  )}
