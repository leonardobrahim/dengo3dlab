...
export const ProductDetailPage: React.FC<ProductDetailPageProps> = ({ slug: propSlug }) => {
  ...
  const [activeTab, setActiveTab] = React.useState<'desc' | 'specs' | 'materials' | 'production' | 'reviews'>('desc');
  const [notifyModalOpen, setNotifyModalOpen] = React.useState(false);
  const [notifyEmail, setNotifyEmail] = React.useState('');

  const currentSlug = propSlug || params.slug || 'mascote-lontrinha-dengo-3d-articulada';
    ...
    return (
      <StoreLayout>
        <NotFoundState
          ...
          actionText="Explorar Todos os Produtos"
      ...
      <div className="space-y-8 text-left max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-6">
            <ProductGallery
              ...
              onToggleFav={() => {
                ...
                toast.success(
                  isFav ? 'Removido dos favoritos' : 'Adicionado aos seus favoritos do Dengo Lab!'
                );
              }}
              onShare={handleShare}
            />
            ...
            <div className="grid grid-cols-3 gap-2 pt-6">
              <div className="flex flex-col items-center text-center p-3 rounded-2xl border border-pink-100 bg-white shadow-2xs space-y-1">
                ...
                <span className="text-[11px] font-bold text-slate-800">100% Eco PLA</span>
                <span className="text-[10px] text-slate-600">Biodegradável</span>
              </div>
              <div className="flex flex-col items-center text-center p-3 rounded-2xl border border-pink-100 bg-white shadow-2xs space-y-1">
                <ShieldCheck className="h-4 w-4 text-pink-500" />
                <span className="text-[11px] font-bold text-slate-800">Garantia 3D</span>
                <span className="text-[10px] text-slate-600">Peça perfeita</span>
              </div>
              <div className="flex flex-col items-center text-center p-3 rounded-2xl border border-pink-100 bg-white shadow-2xs space-y-1">
                <Zap className="h-4 w-4 text-amber-500" />
                <span className="text-[11px] font-bold text-slate-800">Envio Ágil</span>
                <span className="text-[10px] text-slate-600">Pronta entrega</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                >
                  ({reviews.length || product.reviewCount || 38} avaliações de clientes)
                </button>
              </div>
            </div>
            ...
            <div className="p-5 rounded-3xl bg-pink-50/50 border border-pink-200/80 space-y-2 shadow-2xs">
              ...
              <div className="space-y-1 pt-1 text-xs text-slate-600">
                <p className="flex items-center gap-1.5 font-medium">
                  ...
                  <span>
                    ou <strong className="text-slate-800">3x de {formatCurrency(currentPrice / 3)}</strong> sem juros no cartão de crédito
                  </span>
                </p>
                <p className="flex items-center gap-1.5 text-emerald-700 font-bold">
                  <Zap className="h-3.5 w-3.5 text-emerald-600 shrink-0" />
                  <span>5% de desconto extra pagando com PIX ({formatCurrency(currentPrice * 0.95)})</span>
                </p>
              </div>
            </div>
            ...
            <div className="pt-1">
              {isOutOfStock ? (
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-emerald-700 bg-emerald-50/70 p-3 rounded-2xl border border-emerald-200/80">
                  ...
                  <span>✓ Em estoque para envio imediato ({currentStock} unidades disponíveis no Lab)</span>
                </div>
              )}
            </div>
        ...
        <div id="product-tabs-section" className="pt-12 space-y-6">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-pink-100 scrollbar-none">
            {[
              { id: 'desc', label: 'Descrição Completa', icon: <Layers className="h-4 w-4" /> },
              { id: 'specs', label: 'Especificações Técnicas', icon: <Cpu className="h-4 w-4" /> },
              { id: 'materials', label: 'Materiais & Cuidados', icon: <Leaf className="h-4 w-4" /> },
              { id: 'production', label: 'Produção 3D & Prazos', icon: <Clock className="h-4 w-4" /> },
              { id: 'reviews', label: `Avaliações (${reviews.length})`, icon: <Sparkles className="h-4 w-4" /> },
            ].map((tab) => (
              <button
                key={tab.id}
                id={`tab-btn-${tab.id}`}
            ...
            <div className="p-6 sm:p-8 rounded-3xl border border-pink-100 bg-white space-y-6 text-slate-700 shadow-2xs animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                <div className="p-4 rounded-2xl bg-pink-50/50 border border-pink-100 space-y-1.5">
                  ...
                  <h4 className="text-xs font-bold text-slate-800">Acabamento Artesanal</h4>
                  <p className="text-[11px] text-slate-600">
                    Cada peça passa por pós-processamento manual, remoção minuciosa de suportes e inspeção de qualidade óptica.
                  </p>
                </div>

                <div className="p-4 rounded-2xl bg-sky-50/50 border border-sky-100 space-y-1.5">
                  <Leaf className="h-5 w-5 text-sky-600" />
                  <h4 className="text-xs font-bold text-slate-800">Bio-Material Sustentável</h4>
                  <p className="text-[11px] text-slate-600">
                    Produzido com polímeros derivados de amido de milho e cana-de-açúcar. Não tóxico e seguro para toda a família.
                  </p>
                </div>

                <div className="p-4 rounded-2xl bg-amber-50/50 border border-amber-100 space-y-1.5">
                  <ShieldCheck className="h-5 w-5 text-amber-600" />
                  <h4 className="text-xs font-bold text-slate-800">Estrutura Infill Otimizada</h4>
                  <p className="text-[11px] text-slate-600">
                    Preenchimento interno giroidal inteligente que distribui tensões mecânicas para máxima resistência e leveza.
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <tbody>
                    ...
                    <tr className="border-b border-pink-100 bg-pink-50/30">
                      ...
                      <td className="p-3.5 text-slate-800">
                        {product.technicalSpecs?.layerHeightMm || 0.16}mm (Ultra-fino, linhas quase imperceptíveis)
                      </td>
                    </tr>
                    <tr className="border-b border-pink-100">
                      <td className="p-3.5 font-bold text-slate-700">Preenchimento Interno (Infill)</td>
                      <td className="p-3.5 text-slate-800">
                        {product.technicalSpecs?.infillPercent || 20}% Estrutura Giroide Tridimensional
                      </td>
                    </tr>
                    <tr className="border-b border-pink-100 bg-pink-50/30">
                    ...
                    <tr>
                      <td className="p-3.5 font-bold text-slate-700">Tolerância Dimensional</td>
                      <td className="p-3.5 text-slate-800">± 0.1 mm calibrado em impressoras industriais</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          {activeTab === 'materials' && (
            <div className="p-6 sm:p-8 rounded-3xl border border-pink-100 bg-white space-y-6 text-slate-700 shadow-2xs animate-in fade-in duration-200">
              ...
              <div className="space-y-4 text-xs sm:text-sm text-slate-600 leading-relaxed">
                <p>
                  As peças do <strong>Dengo 3D Lab</strong> são fabricadas com polímeros termoplásticos de alta pureza. Para garantir que sua peça dure anos com as cores vivas e textura impecável, siga estas recomendações simples:
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div className="p-4 rounded-2xl border border-pink-100 bg-pink-50/30 space-y-2">
                    ...
                    <ul className="space-y-1 text-xs text-slate-600 list-disc list-inside">
                      <li>Limpar com pano macio levemente umedecido em água ou sabão neutro.</li>
                      <li>Guardar em local arejado, sobre estantes, mesas ou nichos decorativos.</li>
                      <li>Aproveitar as articulações suaves para relaxamento sensorial e fidget.</li>
                    </ul>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl border border-rose-100 bg-rose-50/30 space-y-2">
                    ...
                    <ul className="space-y-1 text-xs text-slate-600 list-disc list-inside">
                      <li>Não expor a temperaturas acima de 55°C (ex: interior de carros fechados no sol).</li>
                      <li>Não utilizar solventes químicos fortes, acetona ou álcool isopropílico puro no PLA Silk.</li>
                      <li>Não levar à máquina de lavar louças ou forno micro-ondas.</li>
                    </ul>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'production' && (
            <div className="p-6 sm:p-8 rounded-3xl border border-pink-100 bg-white space-y-6 text-slate-700 shadow-2xs animate-in fade-in duration-200">
              ...
              <div className="space-y-4 text-xs sm:text-sm text-slate-600 leading-relaxed">
                <p>
                  Diferente do varejo industrial massivo, a <strong>Dengo 3D Lab</strong> combina impressão aditiva de ponta com carinho artesanal maker. Cada modelo é fatiado digitalmente e produzido com velocidade controlada para garantir zero defeitos.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 pt-2">
                  <div className="p-4 rounded-2xl border border-pink-100 bg-white text-center space-y-1.5">
                    ...
                    <h5 className="font-bold text-slate-800 text-xs">Fatiamento Digital</h5>
                    <p className="text-[10px] text-slate-500">Configuração de parâmetros camada a camada no slicer</p>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl border border-pink-100 bg-white text-center space-y-1.5">
                    ...
                    <h5 className="font-bold text-slate-800 text-xs">Impressão 3D 8K</h5>
                    <p className="text-[10px] text-slate-500">Impressão a 300mm/s com calibração de fluxo ativa</p>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl border border-pink-100 bg-white text-center space-y-1.5">
                    ...
                    <h5 className="font-bold text-slate-800 text-xs">Controle de Qualidade</h5>
                    <p className="text-[10px] text-slate-500">Inspeção manual e pós-processamento cuidadoso</p>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl border border-pink-100 bg-white text-center space-y-1.5">
                    ...
                    <h5 className="font-bold text-slate-800 text-xs">Embalagem Candy</h5>
                    <p className="text-[10px] text-slate-500">Caixa decorada com papel de seda, mimos e adesivos</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="pt-12 border-t border-pink-100 space-y-6 text-left">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-600">Modelos 3D complementares da mesma categoria</p>
            </div>
            <Button
              variant="outline"
              size="sm"
        ...
        >
          <form onSubmit={handleNotifySubmit} className="space-y-4 pt-2 text-left">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-700">Seu Melhor E-mail *</label>
              <input
                type="email"
                required
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-pink-100">
              <Button type="button" variant="outline" size="sm" onClick={() => setNotifyModalOpen(false)}>
                Cancelar
              </Button>
