...
export const OrdersPage: React.FC = () => {
  ...
  const filteredOrders = React.useMemo(() => {
    return mockOrders.filter(order => {
      ...
      if (activeTab === 'processing') return ['pending', 'confirmed'].includes(order.status);
      if (activeTab === 'production') return ['in_production', 'ready_to_ship'].includes(order.status);
  ...
  return (
    <AccountLayout currentPageTitle="Meus Pedidos" currentPageBreadcrumb="Meus Pedidos">
      <div className="space-y-6 text-left">
        <div className="space-y-1">
          <h1 className="text-xl sm:text-2xl font-black text-foreground">Histórico de Pedidos</h1>
              ...
              >
                ...
                <div className="p-4 sm:p-5 bg-slate-50 dark:bg-card border-b border-border flex flex-wrap items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">Realizado em {formatDate(order.createdAt)} às {formatTime(order.createdAt)}</p>
                  </div>
                      ...
                      <div key={item.id} className="flex items-center justify-between gap-4 text-xs">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[11px] text-pink-600 font-semibold">{item.quantity}x {formatCurrency(item.unitPrice)}</p>
                          </div>
                        </div>
                        <span className="font-bold text-foreground">{formatCurrency(item.totalPrice)}</span>
                      </div>
                    ))}
                  </div>
                </div>
