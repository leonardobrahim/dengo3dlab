...
import { Package, Heart, MapPin, Sparkles, ArrowRight, Clock } from 'lucide-react';
...
export const AccountOverviewPage: React.FC = () => {
  ...
  return (
    <AccountLayout currentPageTitle="Visão Geral da Conta" currentPageBreadcrumb="Visão Geral">
      <div className="space-y-8 text-left">
        {/* Welcome Banner */}
        <div className="p-6 sm:p-8 rounded-3xl border border-pink-100 bg-gradient-to-r from-pink-50 via-white to-sky-50 space-y-2 shadow-xs">
          ...
          <p className="text-xs sm:text-sm text-slate-600 max-w-xl">
            Acompanhe a impressão e envio dos seus dengos, gerencie endereços e resgate cupons exclusivos.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <Badge variant="candy">2 Pedidos</Badge>
            </div>
            <div>
              <p className="text-xs text-slate-500">Meus Pedidos</p>
              <p className="text-sm font-bold text-slate-900">1 em Produção 3D</p>
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <Badge variant="sky">{productIds.length} Itens</Badge>
            </div>
            <div>
              <p className="text-xs text-slate-500">Lista de Desejos</p>
              <p className="text-sm font-bold text-slate-900">Peças Favoritadas</p>
            </div>
          </div>
            ...
            {recentOrders.map((order) => (
              ...
              >
                <div className="space-y-1">
                  ...
                  <p className="text-xs font-semibold text-slate-900">{order.firstItemName}</p>
                  <p className="text-[11px] text-slate-500 flex items-center gap-1">
                    <Clock className="h-3 w-3" />
                    <span>Realizado em {order.date} • {order.itemCount} item(ns)</span>
                  </p>
                </div>
