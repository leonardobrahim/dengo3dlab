...
export const adminNavItems: NavItem[] = [
  { label: 'Dashboard', path: '/admin/dashboard', alias: '/admin', icon: LayoutDashboard, allowedRoles: ['superadmin', 'admin', 'production', 'stock', 'support'] },
  { label: 'Produtos', path: '/admin/produtos', icon: Package, badge: '6', allowedRoles: ['superadmin', 'admin', 'stock'] },
  { label: 'Categorias', path: '/admin/categorias', icon: Layers, badge: '4', allowedRoles: ['superadmin', 'admin'] },
  { label: 'Pedidos', path: '/admin/pedidos', icon: ShoppingBag, badge: '3', badgeVariant: 'candy', allowedRoles: ['superadmin', 'admin', 'support', 'production', 'stock'] },
  { label: 'Clientes', path: '/admin/clientes', icon: Users, allowedRoles: ['superadmin', 'admin', 'support'] },
  { label: 'Estoque & Insumos', path: '/admin/estoque', icon: Boxes, allowedRoles: ['superadmin', 'admin', 'stock'] },
  { label: 'Fila de Produção 3D', path: '/admin/producao', icon: Cpu, badge: '2 On', badgeVariant: 'sky', allowedRoles: ['superadmin', 'admin', 'production'] },
  { label: 'Envios & Rastreio', path: '/admin/envios', icon: Truck, allowedRoles: ['superadmin', 'admin', 'support', 'stock'] },
  { label: 'Cupons de Desconto', path: '/admin/cupons', icon: Ticket, allowedRoles: ['superadmin', 'admin'] },
  { label: 'Avaliações de Clientes', path: '/admin/avaliacoes', icon: Star, allowedRoles: ['superadmin', 'admin', 'support'] },
  { label: 'Relatórios & Vendas', path: '/admin/relatorios', icon: BarChart3, allowedRoles: ['superadmin', 'admin'] },
  { label: 'Administradores', path: '/admin/administradores', icon: ShieldCheck, allowedRoles: ['superadmin'] },
  { label: 'Configurações', path: '/admin/configuracoes', icon: Settings, allowedRoles: ['superadmin', 'admin'] },
];
...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({ onItemClick }) => {
  ...
  const visibleItems = adminNavItems.filter(item => item.allowedRoles.includes(userRole));
  ...
  return (
    <aside className="w-64 flex flex-col h-full bg-card border-r border-pink-200/60 dark:border-pink-900/40 select-none">
      ...
      <div className="p-5 border-b border-pink-100 dark:border-pink-950/60 flex items-center justify-between">
        <div className="cursor-pointer" onClick={() => handleNav('/admin/dashboard')}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1">
        ...
        {visibleItems.map((item) => {
          ...
          const isActive =
            ...
            (item.path !== '/admin' && item.path !== '/admin/dashboard' && currentPath.startsWith(item.path));
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <Icon className={`h-4 w-4 shrink-0 ${isActive ? 'text-white' : 'text-pink-500'}`} />
